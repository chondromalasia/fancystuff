<?php
	...
	if ($country) {		
		...
		if ($country == "UK" || $country == "Northern Ireland") {
			$result = pg_query ($db, "SELECT COUNT(*) as count FROM fancystuff.protests2 
						  WHERE country like 'UK%' GROUP BY year ORDER BY year ASC");
			$total = pg_query ($db, "SELECT COUNT(*) as count FROM fancystuff.protests2 WHERE country LIKE 'UK%'");
                         $stats = pg_query ($db, "SELECT COUNT(action) AS count, action FROM fancystuff.protests2
                                               WHERE country LIKE 'UK%' GROUP BY action ORDER BY count DESC");
		} 
		else if ($country == "Germany") {
			$result = pg_query ($db, "SELECT COUNT(*) AS count FROM fancystuff.protests2 
						  WHERE country like '%German%' GROUP BY year ORDER BY year ASC");
			$total = pg_query ($db ,"SELECT COUNT(*) AS count FROM fancystuff.protests2 WHERE country LIKE '%German%'");
                        $stats = pg_query ($db, "SELECT COUNT(action) AS count, action FROM fancystuff.protests2
  			...
  			$result = pg_query ($db, "SELECT COUNT(*) AS count FROM fancystuff.protests2 
                        ...
                        $total = pg_query ($db ,"SELECT COUNT(*) AS count FROM fancystuff.protests2 WHERE country LIKE 'Czech%'");
			 $stats = pg_query ($db, "SELECT COUNT(action) AS count, action FROM fancystuff.protests2
                                               WHERE country LIKE 'Czech%' GROUP BY action ORDER BY count DESC");
		}
		else if ($country == "europe") {
			$result = pg_query ($db, "SELECT COUNT(*) AS count, year FROM fancystuff.protests2 
						  GROUP BY year ORDER BY year ASC OFFSET 16");
			$total = pg_query  ($db, "SELECT COUNT(*) AS count FROM fancystuff.protests2");
			$stats = pg_query  ($db, "SELECT COUNT(action) AS count, action FROM fancystuff.protests2
						  GROUP BY action ORDER BY count DESC");
			$events = pg_query ($db, "SELECT COUNT(*) AS count FROM fancystuff.protests2 GROUP BY year ORDER BY count DESC");
		} 
		else {
			$result = pg_query($db, "SELECT COUNT(*) AS count, year FROM fancystuff.protests2 
						 WHERE country='$country' GROUP BY year ORDER BY year ASC");
			$total = pg_query($db,  "SELECT COUNT(*) AS count FROM fancystuff.protests2 WHERE country='$country'");
                        $stats = pg_query ($db, "SELECT COUNT(action) AS count, action FROM fancystuff.protests2
